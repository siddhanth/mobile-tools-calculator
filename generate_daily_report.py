"""
Generate comparison report for top equity mutual funds with DAILY SIP across all strategies
"""
...
def run_daily_fund_simulation(mf_data, pe_data, scheme_name, base_amount=1000):
    """Run daily simulation for a single fund across all strategies"""
    try:
        # MF data is already daily, just align with PE
        mf_data = mf_data.rename(columns={'nav': 'close'})
        ...
        if len(aligned) < 100:  # At least ~4 months of daily data
            return None
...
def main():
    print("=" * 60)
    print("DAILY SIP Strategy Comparison Report Generator")
    print("=" * 60)
    ...
    print("SIP Frequency: DAILY (₹1,000/day)")
    ...
    # Run Nifty baseline with daily SIP
    print("\nRunning Nifty 50 baseline (Daily SIP)...")
    nifty_data = get_nifty_data(start_str, end_str, interval="1d")  # Daily
    nifty_aligned = align_data(nifty_data, pe_data)
    
    daily_base_amount = 1000  # ₹1,000 per day
    
    nifty_results = {}
    for name, strategy in PRESET_STRATEGIES.items():
        result = simulate_sip(nifty_aligned, strategy, daily_base_amount, 
                             price_col='nifty_close', pe_col='pe')
    ...
    # Process each fund
    print(f"\nProcessing {len(TOP_EQUITY_FUNDS)} mutual funds (Daily SIP)...")
    results = []
    ...
    for i, (code, name) in enumerate(TOP_EQUITY_FUNDS.items()):
        ...
        sim_results = run_daily_fund_simulation(mf_data, pe_data, scheme_name, daily_base_amount)
        ...
        results.append({
            ...
            'days': sim_results['balanced']['days']
        })
    ...
    html = generate_html_report(df, nifty_results, "Daily")
    
    report_path = "/Users/siddharthjain/Documents/Sid/sip_simulator/daily_fund_comparison_report.html"
    ...
    # Also save CSV
    csv_path = "/Users/siddharthjain/Documents/Sid/sip_simulator/daily_fund_comparison_data.csv"
