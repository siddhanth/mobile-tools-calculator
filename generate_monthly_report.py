"""
Generate comparison report for top equity mutual funds with MONTHLY SIP across all strategies
"""

import pandas as pd
import requests
from datetime import datetime, timedelta
from data_fetcher import get_nifty_pe_data, align_data, get_nifty_data
...
def fetch_mf_nav(scheme_code: str, start_date: str, end_date: str):
    """Fetch MF NAV data from mfapi.in (daily data)"""
    try:
...
def resample_to_monthly(df):
    ...
    monthly.columns = ['date', 'nav']
    return monthly


def run_monthly_fund_simulation(mf_data, pe_data, scheme_name, base_amount=21650):
    """Run monthly simulation for a single fund across all strategies"""
    try:
        # Resample MF data to monthly
        mf_monthly = resample_to_monthly(mf_data)
        mf_monthly = mf_monthly.rename(columns={'nav': 'close'})
        
        # Merge with PE data (get end of month PE)
        pe_monthly = pe_data.copy()
        pe_monthly = pe_monthly.set_index('date')
        pe_monthly = pe_monthly[['pe']].resample('M').last()
        pe_monthly = pe_monthly.dropna().reset_index()
        ...
        aligned = aligned.rename(columns={'close': 'nifty_close'})
        
        if len(aligned) < 12:  # At least 1 year of monthly data
            return None
...
def generate_html_report(results_df, nifty_results, sip_type="Daily"):
...
<head>
    ...
    <title>{sip_type} SIP Strategy Comparison - Top 50 Equity Funds</title>
    <style>
        * {{ box-sizing: border-box; margin: 0; padding: 0; }}
        ...
        .card h3 {{ color: #f59e0b; margin-bottom: 10px; }}
        ...
        .nifty-baseline h3 {{ color: #f59e0b; margin-bottom: 15px; }}
        .nifty-grid {{
            display: grid;
        ...
        th:hover {{ background: linear-gradient(135deg, #d97706 0%, #dc2626 100%); }}
        td {{
            padding: 12px 10px;
...
<body>
    <div class="container">
        <h1>📅 {sip_type} SIP Strategy Comparison <span class="badge">{sip_type}</span></h1>
        <p class="subtitle">Top Equity Mutual Funds | 5-Year Backtest | ₹1,000/day SIP</p>
        
        <div class="nifty-baseline">
            <h3>📊 Nifty 50 Baseline ({sip_type} SIP)</h3>
...
def main():
    print("=" * 60)
    print("MONTHLY SIP Strategy Comparison Report Generator")
    print("=" * 60)
    ...
    print("SIP Frequency: MONTHLY (₹21,650/month)")
    ...
    # Run Nifty baseline with monthly SIP
    print("\nRunning Nifty 50 baseline (Monthly SIP)...")
    nifty_data = get_nifty_data(start_str, end_str, interval="1mo")  # Monthly
    nifty_aligned = align_data(nifty_data, pe_data)
    
    monthly_base_amount = 21650  # ₹21,650 per month (₹5000/week * 4.33)
    
    nifty_results = {}
    for name, strategy in PRESET_STRATEGIES.items():
        result = simulate_sip(nifty_aligned, strategy, monthly_base_amount, 
                             price_col='nifty_close', pe_col='pe')
    ...
    # Process each fund
    print(f"\nProcessing {len(TOP_EQUITY_FUNDS)} mutual funds (Monthly SIP)...")
    results = []
    ...
    for i, (code, name) in enumerate(TOP_EQUITY_FUNDS.items()):
        ...
        if mf_data is None or len(mf_data) < 200:
            print("SKIPPED (insufficient data)")
            continue
        
        sim_results = run_monthly_fund_simulation(mf_data, pe_data, scheme_name, monthly_base_amount)
        ...
        results.append({
            ...
            'months': sim_results['balanced']['months']
        })
    ...
    html = generate_html_report(df, nifty_results, "Monthly")
    
    report_path = "/Users/siddharthjain/Documents/Sid/sip_simulator/monthly_fund_comparison_report.html"
    ...
    # Also save CSV
    csv_path = "/Users/siddharthjain/Documents/Sid/sip_simulator/monthly_fund_comparison_data.csv"
